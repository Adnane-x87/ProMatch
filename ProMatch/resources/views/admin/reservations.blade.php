    ...
    <div x-data="{ search: '', status: '', date: '' }">
        ...
        <div class="bg-white p-4 rounded-xl border border-slate-200 flex flex-wrap gap-4 items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <input x-model="date" type="date" class="px-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-brand-500 text-slate-600">
            </div>
            <div class="flex items-center gap-2">
                <button @click="search = ''; status = ''; date = ''" class="p-2 text-slate-400 hover:text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50" title="Réinitialiser les filtres">
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden mt-6">
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50"
                            x-data="{ 
                                name: '{{ strtolower(addslashes($reservation->first_name . ' ' . $reservation->last_name)) }}',
                                phone: '{{ $reservation->phone }}',
                                resStatus: '{{ $reservation->status === 'CONFIRMED' ? 'APPROVED' : $reservation->status }}',
                                resDate: '{{ substr($reservation->request_date, 0, 10) }}'
                            }"
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                <p class="text-sm text-slate-500">Affichage de <span class="font-medium text-slate-900">1</span> à <span class="font-medium text-slate-900">{{ count($reservations) > 10 ? 10 : count($reservations) }}</span> sur <span class="font-medium text-slate-900">{{ count($reservations) }}</span> résultats</p>
